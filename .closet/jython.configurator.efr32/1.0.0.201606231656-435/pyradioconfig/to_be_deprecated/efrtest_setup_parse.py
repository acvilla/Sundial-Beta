...
def print_python_source(phy_name, model, source_lines):
    ...
    # There is one phy that has a different clock frequency.  Just special case it here.
    if (model.profile.inputs.xtal_frequency.var_value != 38400000):
...
def extract_to_source():
    ...
    source_lines.append("from pyradioconfig.pycalcmodel import ModelInput, ModelOutput")
    ...
    current_path = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(current_path, '../../parts/dumbo/phys')
    output_dir = 'C:/validation/host_py_radio_config/Package/pyradioconfig/parts/dumbo/phys'
