...
class CALC_Demodulator(ICalculator):
    ...
    def calc_baudrate(self, model):
        ...
        bitrate_offset = model.vars.rx_bitrate_offset_hz.value

        # for 4-FSK baudrate is half of bitrate
        if modformat == model.vars.modulation_type.var_enum.FSK4:
            baudrate = bitrate / 2.0
            baudrate_offset = bitrate_offset / 2.0
        # in DSSS mode factor in the spreading_factor to get baudrate = chiprate
        elif spreading_factor > 0:
            baudrate = bitrate * spreading_factor
            baudrate_offset = bitrate_offset * spreading_factor
        # otherwise baudrate is equal to bitrate
        else:
            baudrate = bitrate
            baudrate_offset = bitrate_offset
            ...
            for dec1 in xrange(32768, 0, -1):
                ...
                for cfosr in cfosr_list:
                    ...
                    for dec2 in xrange(dec2_start, dec2_end + 1):
                        ...
                        #print dec0, dec1, dec2, cfosr, bw, baudrate, fc
    ...
    def calc_cfosr_reg(self, model):
        ...
        if val == 7:
            ...
            reg = 99
            print "Undefined value for CFOSR!"
    ...
    def calc_bw_carson(self, model):
        ...
        if mod_format == model.vars.modulation_type.var_enum.OOK or \
           mod_format == model.vars.modulation_type.var_enum.ASK:
            bw_carson = 2.0 * baudrate_hz + freq_tolerance
        else:
            bw_carson = baudrate_hz + 2.0 * freq_dev_hz + freq_tolerance

        model.vars.bandwidth_carson_hz.value = int(round(bw_carson))
        # default value of bandwidth_hz is the carson bandwidth
        model.vars.bandwidth_hz.value = int(round(bw_carson))
    ...
    def calc_if_frequency_actual(self, model):
        ...
        model.vars.if_frequency_hz_actual.value = if_frequency
    ...
    def calc_mod_type_reg(self, model):
        ...
        if modformat == model.vars.modulation_type.var_enum.FSK2:
            ...
            print 'ERROR: modulation method in input file not recognized'
    ...
    def calc_interpolation_gain_actual(self, model):
        ...
        model.vars.interpolation_gain_actual.value = interpolation_gain
    ...
    def calc_baudrate_tol_ppm_actual(self, model):
        ...
        model.vars.baudrate_tol_ppm_actual.value = int(round(tol))
