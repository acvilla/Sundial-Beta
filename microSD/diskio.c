...
)
{
  ...
  GPIO_PinOutSet(MICROSD_GPIOPORT, MICROSD_CSPIN); /* CS pin high. */
  ...
  while (MICROSD_SendCmd(CMD0, 0) != 1 && !MICROSD_TimeOutElapsed()){
	  USTIMER_DelayIntSafe(1000); //repeat until proper response is received
  } /* Enter Idle state */

GPIO_PinOutClear(MICROSD_GPIOPORT, MICROSD_CSPIN); /* CS pin low. */

int resp = MICROSD_SendCmd(CMD8, 0x1AA);
    if (resp == 1) {    /* SDv2? */
      for (n = 0; n < 4; n++) ocr[n] = MICROSD_XferSpi(0xff); /* Get trailing return value of R7 resp */

      if (ocr[2] == 0x01 && ocr[3] == 0xAA) {   /* The card can work at vdd range of 2.7-3.6V */
            while (!MICROSD_TimeOutElapsed() && MICROSD_SendCmd(ACMD41, 0x40000000)){
